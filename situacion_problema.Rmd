...
# Archivos por año
archivos <- c("alfa2020.fasta", "alfa2021.fasta")
años <- c(2020, 2021)
...
# Filtrar y agrupar en un solo paso
longitudes_df <- longitudes_df %>%
  filter(!is.na(gen), gen != "") %>%
  group_by(gen) %>%
  summarise(longitud = max(longitud), .groups = "drop")

# Verificar coincidencias con el conteo
unique(longitudes_df$gen)
setdiff(unique(conteo$gen), longitudes_df$gen)

```

```{r}
# Calcular densidad por gen
densidad <- conteo %>%
  left_join(longitudes_df, by = "gen") %>%
  mutate(densidad = muts / longitud)

# Clasificar grupo de genes
genes_estructurales <- c("S", "N", "M", "E")
genes_no_estructurales <- c("ORF1AB", "ORF3A", "ORF6", "ORF7A", "ORF7B", "ORF8", "ORF10")
...
# Calcular densidad promedio por grupo y año
densidad_promedio <- densidad %>%
  group_by(año, grupo) %>%
  summarise(densidad_prom = mean(densidad), .groups = "drop")

# Gráfica final
ggplot(densidad_promedio, aes(x = año, y = densidad_prom * 1000, color = grupo)) +
  geom_line(size = 1.3) +
  geom_point(size = 3) +
  labs(title = "Densidad promedio de mutaciones no sinónimas por grupo (por 1000 nt)",
       x = "Año", y = "Mutaciones por mil nucleótidos", color = "Grupo") +
  theme_minimal(base_size = 14) +
  theme(plot.title = element_text(face = "bold", hjust = 0.5))
...
```{r}
ggplot(densidad %>% group_by(año, grupo) %>%
         summarise(densidad_prom = mean(densidad), .groups = "drop"),
       aes(x = año, y = densidad_prom, color = grupo)) +
  geom_line(size = 1.3) +
  geom_point(size = 3) +
  labs(title = "Densidad de mutaciones no sinónimas por grupo (ajustada por longitud)",
       x = "Año", y = "Mutaciones por nucleótido", color = "Grupo") +
  theme_minimal(base_size = 14)
```
